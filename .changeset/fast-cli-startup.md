...
- New `politty generate-shim` CLI command (and `generateCompileCacheShim` export) that generates the bin shim as part of a `postbuild`/`prepack` script, deriving the program name from package.json.
