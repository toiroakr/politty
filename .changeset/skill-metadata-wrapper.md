...
- Installer populates `.agents/skills/<name>` (canonical) and each agent-specific dir (e.g. `.claude/skills/<name>`) from the source; the materialization is controlled by `mode`: `"auto"` (default — try symlink, fall back to copy on Windows without Developer Mode and other filesystems without symlink support), `"symlink"` (throws on symlink failure), or `"copy"` (recursive copy). Symlink-mode installs propagate source updates live; copy-mode installs are snapshots that require re-running `sync`. Agent-specific slots route through the canonical slot so one `sync` swaps all hops at once
