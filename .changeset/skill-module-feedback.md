...
- `skills sync --exclude` now warns on stderr when an exclude value doesn't match any skill in the source directory (e.g. typos like `--exclude nonexitent`). Sync still proceeds — a stray exclude is a no-op, not a hard stop — but the warning surfaces inert flags instead of dropping them silently.
