...
`--entry` is the specifier the shim imports, relative to the shim file. Everything else is derived from `package.json`: the output path is the `bin` path (that is where the executable must live), the program name for the cache directory is the `bin` name, and the package the shim imports `enableCompileCache` from is the politty package you depend on — the first of `@politty/zod`, `@politty/valibot`, `politty` found in `dependencies`, `peerDependencies` or `devDependencies`. `--entry` itself can also be omitted — the generator then picks the first of `./cli.js`, `./cli.mjs`, `./index.js`, `./index.mjs` that exists next to the shim. Override the derived values with `--out`, `--program` and `--compile-cache-specifier` when needed.

The shim runs inside your package, so only a politty package your own `package.json` declares is guaranteed to resolve from it — that is why the specifier is derived rather than fixed. If none is declared, the generator warns and falls back to `politty/compile-cache`.
...
Writing the shim by hand is equally fine — import from the politty package your CLI depends on:
