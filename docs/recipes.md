...
The shim imports `enableCompileCache` from **the politty package whose `politty` bin you ran**: `@politty/zod/compile-cache`, `@politty/valibot/compile-cache`, or `politty/compile-cache`. Running that bin means the package is installed in your project, so the specifier resolves from the shim it writes there. Pass `--compile-cache-specifier` if politty reaches your code some other way, such as a re-export from your own package.
...
Writing the shim by hand is equally fine — import from the politty package your CLI depends on (`@politty/valibot` here; use whichever you installed):

```typescript
#!/usr/bin/env node
// bin.ts — keep this file's static imports minimal
import { enableCompileCache } from "@politty/valibot/compile-cache";
