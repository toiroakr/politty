...
| `customizable`   | `boolean`                | Allow hand-editing `files` output, preserved via markers (default `false`) |

### Fully generated `files` output (default)

`files` output is **fully generated by default**: the files contain no
`<!-- politty:...:start/end -->` markers and are regenerated as a whole on every
run, so you should not hand-edit them. With `targetCommands`, only files that
contain a target command are processed, but each such file is still rebuilt in
full.

Set `customizable: true` when you want to hand-edit the output and have politty
preserve your edits. Generated sections are then wrapped in markers and
located/updated in place (with `targetCommands`), keeping handwritten content
between them. This is also required when a single `files` document interleaves
generated sections with handwritten content, or when you rely on per-command
incremental updates (for example, populating one command's executed-example
output at a time).

When `customizable` is set and the generated output for a command contains a
section the file does not yet have, it is reported as a **non-fatal warning**.
Run with `POLITTY_DOCS_DOCTOR=true POLITTY_DOCS_UPDATE=true` to insert the
missing section, or leave it removed to opt that section out — doctor mode is not
forced on, so deliberately removed sections are not re-inserted.

This flag does not affect `path`/`rootDoc` output — which always uses markers to
inject generated sections into a handwritten host file — nor `templates`, which
...
- When `targetCommands` is specified, only files that contain one of those commands are processed
- In the default marker-free mode, each processed file is regenerated as a whole
- **Subcommand recursive expansion**: Subcommands of specified commands are also automatically included
...
With `customizable: true`, `targetCommands` instead updates only the matching
command sections in place and preserves the rest:

- Only the target command sections are generated/validated
- Other command sections are preserved as-is if they exist in the file
- A command with no markers yet has its full section set inserted
- A newly added section on an already-marked command is reported as a warning (insert it with doctor mode, or leave it out)
