...
| `POLITTY_DOCS_DOCTOR` | Overrides missing-marker detection for a single run (`false`/`0` to skip, `true`/`1` to force) |

### Missing-Marker Detection (Doctor)

Sections that exist in the generated output but have no markers in the documentation file (for example, a newly added `notes` field, or new section types introduced by a politty upgrade) are detected **by default**. In read-only mode they are reported as errors; in update mode (`POLITTY_DOCS_UPDATE=true`) the missing markers are inserted automatically.

Detection applies to marker-based comparison (partial validation) — when `targetCommands` is specified or for the PathConfig root document. Full-file comparison regenerates the whole file, so missing-marker drift cannot occur there.

#### Opting out of a section

To intentionally omit a section from the docs, use a custom renderer that returns an empty string — sections absent from the generated output are never flagged:
...
await assertDocMatch({
  ...
  targetCommands,
  format: {
    renderNotes: () => "", // omit notes sections everywhere
  },
});
```

Renderers receive the command info, so per-command opt-out is also possible (e.g. `renderNotes: (ctx) => (ctx.info.commandPath === "foo" ? "" : ctx.content)`).

#### Disabling detection

To restore the legacy behavior where missing markers are silently skipped (treating marker removal as an opt-out), set `doctor: false` in the config:

```typescript
await assertDocMatch({ command, files, targetCommands, doctor: false });
```

The `POLITTY_DOCS_DOCTOR` env var overrides the config option for a single run:

```bash
POLITTY_DOCS_DOCTOR=false pnpm test            # skip detection once
POLITTY_DOCS_DOCTOR=true POLITTY_DOCS_UPDATE=true pnpm test  # force insert even with doctor: false
```

#### Behavior details

- **Content preservation**: When both adjacent markers exist (preceding and following), insertion wraps the existing content between them with the new markers, preserving user customizations
