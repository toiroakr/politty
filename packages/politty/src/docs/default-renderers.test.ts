...
describe("default-renderers", () => {
  ...
  describe("createCommandRenderer", () => {
    ...
    it("should render options via the configured style with no markers", async () => {
      const cmd = defineCommand({
        name: "test",
        args: z.object({
      ...
      const info = await buildCommandInfo(cmd, "test");
      const markdown = createCommandRenderer({ optionStyle: "list" })(info);

      expect(markdown).toContain("- `-v`");
      expect(markdown).not.toContain("| Option |");
      // Output is markerless: no command/section markers.
      expect(markdown).not.toContain("<!-- politty:");
    });
  });
