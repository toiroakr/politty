 ...
 * Startup"): a shim that enables the Node.js on-disk compile cache and then
 * loads the real CLI with a dynamic import. Meant to be wired into a
 * `postbuild` or `prepack` script via `politty generate-shim` so the shim
 * never has to live in source.
 *
 * Each politty package binds its own generator with
 * {@link createCompileCacheShimGenerator}, so the shim imports the cache
 * helper from the package that generated it.
 */
...
/**
 * Options for the generator returned by
 * {@link createCompileCacheShimGenerator} — exported as
 * `generateCompileCacheShim` from each politty package.
 */
export interface GenerateCompileCacheShimOptions {
   ...
   * the `compile-cache` subpath of the politty package this generator belongs
   * to (see {@link createCompileCacheShimGenerator}). Set this when politty is
   * reached some other way — a re-export from your own package, an import
   * map, and so on.
   */
...
/**
 * One generated shim from the generator returned by
 * {@link createCompileCacheShimGenerator}.
 */
...
/**
 * Build the shim generator for one politty package.
 *
 * `ownerPackage` is the npm name of the package the generator is exported
 * from (`@politty/zod`, `@politty/valibot`, `politty`), and the shim imports
 * `enableCompileCache` from its `compile-cache` subpath. Taking the name from
 * the owner rather than guessing it is what makes the specifier reliable:
 * reaching this generator at all — through that package's bin or through an
 * import of it — means the package is installed in the host, so the specifier
 * resolves from the shim that lands there. The alternative, reading the host's
 * `package.json`, could only guess which of the three politty packages was
 * meant.
 *
 * The returned function generates executable shims. With no explicit paths,
 * one is generated per `bin` entry of the nearest `package.json`, each
 * importing a conventional built module next to it — so a bare
 * `politty generate-shim` in a `postbuild` script is usually enough. Explicit
 * `entry` values are paired in order with the `bin` entries (or with explicit
 ...
 */
export function createCompileCacheShimGenerator(
  ownerPackage: string,
): (options?: GenerateCompileCacheShimOptions) => GenerateCompileCacheShimResult[] {
  return (options = {}) => generateShims(`${ownerPackage}/compile-cache`, options);
}

function generateShims(
  defaultCompileCacheSpecifier: string,
