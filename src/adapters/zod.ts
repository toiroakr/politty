 ...
 * Zod schema adapter.
 *
 * Introspects Zod schemas via their native `_def` structure (Zod is imported
 * only as a type, so this module pulls no Zod code into the runtime bundle) and
 * validates via `safeParse`. Registered with the core registry by importing the
 * `politty/zod` entrypoint.
 */

import type { z } from "zod";
import { getArgMeta as getArgMetaFromRegistry, type ArgMeta } from "../core/arg-registry.js";
import {
  buildFieldMeta,
...
/**
 * Zod native introspection (`_def` walk).
 */
...
/**
 * Schema adapter for Zod. Register it by importing `politty/zod`.
 */
